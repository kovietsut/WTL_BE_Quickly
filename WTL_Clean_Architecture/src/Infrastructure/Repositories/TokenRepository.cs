...
namespace Infrastructure.Repositories
{
        ...
        IUserRepository iUserRepository) : RepositoryBase<AuthMethod, string, MyDbContext>(dbContext, unitOfWork), ITokenRepository
    {
        ...
        {
            var expiration = DateTime.Now.AddMinutes(Convert.ToDouble(configuration["Jwt:Lifetime"]));
        ...
        {
            var currentUser = await iUserRepository.GetUserByEmail(model.Email) ?? throw new Exception("User not found");
            var role = currentUser.RoleId;
        ...
        {
            var currentUser = await iUserRepository.GetUserByEmail(email) ?? throw new Exception("User not found");
            var role = currentUser.RoleId;
            var jwtTokenHandler = new JwtSecurityTokenHandler();
            var secretKey = configuration["Jwt:Secret"];
            if (string.IsNullOrEmpty(secretKey))
